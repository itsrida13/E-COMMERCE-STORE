...
function CheckoutContent() {
  ...
  const [savedCoupon, setSavedCoupon] = useState(() => sessionStorage.getItem("savedCoupon"));
  ...
  const handleApplyCoupon = (e) => {
    ...
    if (code === "SAVE10" || code === "SAVE20") {
      setSavedCoupon(code);
      sessionStorage.setItem("savedCoupon", code);
      setCouponInput("");
    } else {
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      ...
      // Clear coupon on successful checkout
      sessionStorage.removeItem("savedCoupon");
      
      // Instead of creating the order immediately, navigate to Payment Screen
      navigate("/payment", { 
        state: { items, cartItems: cart, finalTotal, shippingAddress } 
      });
    } catch (err) {
  ...
  return (
    <div className="page checkout-page">
      <div className="container">
        ...
        <form onSubmit={handlePlaceOrder} className="checkout-form">
          ...
          <section className="checkout-section card">
            ...
            <p className="order-total-line">Subtotal: ${cartTotal.toFixed(2)}</p>
            {savedCoupon && discountAmount > 0 && (
              <p className="order-discount-line">
                Coupon ({savedCoupon}): <span className="discount-amount">-${discountAmount.toFixed(2)}</span>
              </p>
            )}
            ...
            <div className="coupon-entry">
               ...
               />
               <button type="button" onClick={handleApplyCoupon} className="btn-apply-coupon">Apply</button>
            </div>
      ...
      <style>{`
        ...
        .coupon-entry input:focus { outline: none; border-color: #be185d; }
        .btn-apply-coupon {
          padding: 8px 16px;
          background: #fbcfe8;
          color: #9d174d;
        ...
        .btn-apply-coupon:hover { background: #f9a8d4; }
        .coupon-error-msg { font-size: 12px; color: #dc2626; margin-top: 4px; }
        .order-total-line, .order-discount-line { display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 15px; }
        .order-discount-line { color: #059669; font-weight: 500; }
      `}</style>
    </div>
  );
}
