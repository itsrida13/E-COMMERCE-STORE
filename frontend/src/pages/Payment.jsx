...
export default function Payment() {
  ...
  const handlePayment = async (e) => {
    ...
    const customerName = state.shippingAddress.fullName;
    const email = state.shippingAddress.email;
    const phone = state.shippingAddress.phone;
    ...
    const unpackedOrderItems = [];
    const unpackedItems = [];
    const bundlesMetadata = [];

    state.cartItems.forEach((item) => {
      if (item.product.isBundle) {
        const bundlePrice = Number(item.product.bundlePrice || 0);
        const qty = Number(item.quantity || 1);

        bundlesMetadata.push({
        ...
        const subProducts = item.product.products || [];
        const sumOfListPrices = subProducts.reduce((sum, p) => sum + Number(p.price || 0), 0);

        subProducts.forEach((p) => {
          const listPrice = Number(p.price || 0);
          const proportion = sumOfListPrices > 0 ? (listPrice / sumOfListPrices) : (1 / subProducts.length);
          const splitPrice = Math.round(bundlePrice * proportion * 100) / 100;

          unpackedOrderItems.push({
        ...
        const qty = Number(item.quantity || 1);
        const activePrice = item.product.isDiscounted && item.product.discountedPrice > 0 
    ...
    setTimeout(async () => {
      try {
        ...
        clearCart();
        navigate("/orders", {
          state: { message: "Payment successful! Order placed." },
        });
      } catch (err) {
        setError(
          err.response?.data ||
            ...
            <form onSubmit={handlePayment} className="mock-payment-form">
              ...
              {paymentMethod === "paypal" && (
                <div className="paypal-info animate-in">
                  <p>
                    ...
                    your purchase after clicking the button below.
                  </p>
                </div>
              )}
              ...
              {paymentMethod === "cod" && (
                <div className="cod-info animate-in">
                  <p>
                    Please have the exact amount ready when our delivery partner
              ...
              <button disabled={loading} className="btn btn-primary submit-btn">
                {loading
                  ? "Processing Payment..."
                  : `Pay $${state.finalTotal.toFixed(2)}`}
              </button>
            </form>
          </div>
        </div>
