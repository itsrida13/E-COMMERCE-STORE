...
exports.handleChatRequest = async (req, res) => {
  try {
    ...
    const searchTerms = text.replace(/under\s*\$?\s*\d+/g, "")
      .replace(/(show me|find|looking for|i want|cheap|best|all|products|items|things)/gi, "")
      .trim().split(" ").filter(word => word.length > 1);
    ...
    if (Object.keys(query).length > 0) {
      ...
      if (products.length > 0) {
        return res.json({
          reply: `I found some products that match your request!`,
          intent: "product_search",
          products: products
        });
      }
    } else if (text.includes("products") || text.includes("items") || text.includes("all")) {
      const products = await Product.find().limit(5);
      if (products.length > 0) {
        return res.json({
          reply: `Here are some of our popular products! You can view the entire catalog on our Products page.`,
          intent: "product_search",
          products: products
        });
      }
